...
void aggiungi_stazione() {
    ...
    if (in_station_position(station_tree, distance_input)==NULL)
        ...
        station new_station = malloc(sizeof(Node) + num_input * sizeof(int));
...
void aggiungi_auto(){
    if (station_tree==NULL)
        ...
        int search_distance;
        scanf("%d", &search_distance);
        if (in_station_position(search_distance) == 1) {
            while (station_tree->distance != search_distance)
                if (search_distance < station_tree->distance)
                    station_tree = station_tree->sx;
                else station_tree = station_tree->dx;
        }
        if (station_tree->size == station_tree->capacity) {
            station_tree->capacity *= 2;
            station_tree->car = (int*) realloc(sizeof(int)*(station_tree->capacity));
        }
    }
}
